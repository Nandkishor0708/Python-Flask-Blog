...
class Contacts(db.Model):
    ...
    PhNo  = db.Column(db.String(120), unique=False, nullable=False)
...
class Posts(db.Model):
    ...
    byUser  = db.Column(db.String(20), unique=False, nullable=False)
    date    = db.Column(db.String(12), unique=False, nullable=True)
...
def edit(sno):
    if 'user' in session and session['user']==params['admin_user']:
        if request.method == 'POST':
            ...
            b_tline = request.form.get('tline')
            b_slug = request.form.get('slug')
            b_content = request.form.get('content')
            b_Img_file = request.form.get('img_file')
            b_byUser = params['admin_user']
            date = datetime.now()

            if sno == '0':
                post = Posts(title=b_title, tagline=b_tline, slug=b_slug, content=b_content,
                            img_file=b_Img_file, byUser=b_byUser,date=date)
                db.session.add(post)
                db.session.commit()
            else:
                post = Posts.query.filter_by(sno=sno).first()
                post.title=b_title
                post.tagline=b_tline
                post.slug=b_slug
...
def contact():
    if request.method == "POST":
        ...
        entry = Contacts(Name=name, email=email, PhNo=phone, Mes=message, date=datetime.now())
